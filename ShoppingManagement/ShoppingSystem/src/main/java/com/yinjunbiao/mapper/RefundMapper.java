...
public interface RefundMapper {
    ...
    List<Refund> selectByOrderId(@Param("orderId")String orderId);

    @Select(sql = "select * from refund where status = 1 order_id = #{orderId}")
    Refund selectSuccessByOrderId(@Param("orderId")String orderId);

    @Select(sql = "select * from refund where status = 2 order_id = #{orderId}")
    Refund selectFailedByOrderId(@Param("orderId")String orderId);

    @Insert(sql = "insert into refund values(null,#{orderedId},#{cause},0,#{description})")
    int insert(@Param("orderedId")String orderedId,@Param("cause")Integer cause,@Param("description")String description);
    ...
    int updateCause(@Param("cause")Integer cause,@Param("id")Integer id);
